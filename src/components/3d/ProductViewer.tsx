...
const ChairModel = dynamic(
  () =>
    import("./models/ChairModel").then((mod) => ({ default: mod.ChairModel })),
  { ssr: false },
);
const LampModel = dynamic(
  () =>
    import("./models/LampModel").then((mod) => ({ default: mod.LampModel })),
  { ssr: false },
);
const VaseModel = dynamic(
  () =>
    import("./models/VaseModel").then((mod) => ({ default: mod.VaseModel })),
  { ssr: false },
);
const RingModel = dynamic(
  () =>
    import("./models/RingModel").then((mod) => ({ default: mod.RingModel })),
  { ssr: false },
);
const SculptureModel = dynamic(
  () =>
    import("./models/SculptureModel").then((mod) => ({
      default: mod.SculptureModel,
    })),
  { ssr: false },
);
...
export function ProductViewer({
  ...
  const materialProps = {
    roughness:
      materialVariant?.id === "matte"
        ? 0.8
        : materialVariant?.id === "glossy"
          ? 0.1
          : materialVariant?.id === "metallic"
            ? 0.2
            : materialVariant?.id === "wood"
              ? 0.7
              : 0.5,
    metalness:
      materialVariant?.id === "metallic"
        ? 0.9
        : materialVariant?.id === "chrome"
          ? 1.0
          : materialVariant?.id === "brushed"
            ? 0.7
            : 0.1,
  };
